...
* ---------------------------------------------------------------- */
let GotapiPluginLoader = function(config) {
	this.plugin_root_path = config.plugin_root_path;
	this.plugins = {};
	this.MessageChannel = require('./mesage-channels.js');
	this.PluginUtil = require('./gotapi-plugin-util.js');
};
...
GotapiPluginLoader.prototype._loadPlugins = function(plugin_module_path_list, callback) {
	...
	try {
		...
		let message_channel = new this.MessageChannel();
		let plugin_util = new this.PluginUtil(message_channel.port2);
...
GotapiPluginLoader.prototype._getPluginModulePathList = function(module_search_path_list) {
	...
	module_search_path_list.forEach((module_search_path) => {
		if(fs.existsSync(module_search_path) && fs.statSync(module_search_path).isDirectory()) {
			let entry_list = fs.readdirSync(module_search_path);
			entry_list.forEach((name) => {
				if(name.match(/^node\-gotapi\-plugin\-([a-zA-Z].*)$/)) {
					module_path_list.push(path.join(module_search_path, name));
				}
			});
		}
	});

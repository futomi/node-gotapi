...
let GotapiInterface1 = function(config, sendToGotapiServer) {
	...
	this.requests = {};
	this.http_request_timeout = 60; // Seconds
...
GotapiInterface1.prototype._receiveRequestFromApp = function(req, res) {
	...
	if(ip_allowed) {
		if(this._checkOrigin(req)) {
			...
			this._retun403ToApp(req, res, 'The access from the origin `' + req.headers['origin'] + '` is not allowed.');
		}
	} else {
		this._retun403ToApp(req, res, 'The access from the IP address `' + req.connection.remoteAddress + '` is not allowed.');
	}
};
...
GotapiInterface1.prototype._handleRequestFromApp = function(req, res) {
	...
	if(params_parse_result['error']) {
		this._returnErrorToApp(request_id, 400, 'Failed to parse parameters: ' + params_parse_result['error']);
		return;
	}
	if(url_path_parts[1] === 'gotapi') {
		...
		if(ctype === 'application/x-www-form-urlencoded') {
			...
			req.on('end', () => {
				...
				if(p['error']) {
					this._returnErrorToApp(request_id, 400, 'Failed to parse parameters: ' + p['error']);
					return;
				}
				if(p['result']) {
				...
				this.sendToGotapiServer(this.requests[request_id]['message']);
			});
		} else if(ctype.match(/^multipart\/form\-data/)) {
			this._returnErrorToApp(request_id, 400, 'The content type `multipart/form-data` is not supported.');
		} else {
			this.sendToGotapiServer(this.requests[request_id]['message']);
		}
	} else {

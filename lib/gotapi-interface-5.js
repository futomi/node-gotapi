...
* ---------------------------------------------------------------- */
'use strict';
let mWebsocket = require('websocket.io');
...
let GotapiInterface5 = function(config, if1_server, sendToGotapiServer) {
	...
	this.server = mWebsocket.attach(if1_server);
...
GotapiInterface5.prototype.start = function() {
	var promise = new Promise((resolve, reject) => {
		this.server.on('connection', (ws_io_conn) => {
			if(this.server.clientsCount > this.config.ws_connection_limit) {
				ws_io_conn.close();
			} else if(ws_io_conn.req.url !== '/gotapi/websocket') {
				ws_io_conn.close();
			} else {
				this._connected(ws_io_conn);
			}
		});
...
GotapiInterface5.prototype._returnResponseToApp = function(data) {
	...
	if(data['result'] === 0) {
		...
		data['errorText'] = mHttp.STATUS_CODES[data['errorCode']] || 'Unknown Error';
